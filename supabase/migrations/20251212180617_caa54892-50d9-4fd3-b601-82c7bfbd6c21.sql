-- Create a function to atomically update a single scene's imageUrl
-- This prevents race conditions when multiple webhooks update simultaneously
CREATE OR REPLACE FUNCTION public.update_scene_image_url(
  p_project_id uuid,
...
BEGIN
  ...
  -- Update the specific scene's imageUrl atomically
  updated_prompts := jsonb_set(
    current_prompts,
    ARRAY[p_scene_index::text, 'imageUrl'],
