...
def generate_zoom_frames_cupy_streaming(
    image_path: str,
    width: int,
    height: int,
    framerate: int,
    ...
    """
    Generate zoom frames using CuPy GPU and YIELD them one by one (generator).
    This allows streaming to FFmpeg without storing all frames in RAM.
    Yields: numpy arrays of shape (height, width, 3) in BGR format.
    """
    ...
    # Generate and YIELD frames one by one (streaming, no RAM accumulation)
    for i in range(total_frames):
        ...
        # Transfer to CPU and yield immediately (NO disk I/O, NO RAM accumulation)
...
def generate_zoom_frames_opencv(
    ...
    # Resize to target dimensions with Lanczos4 (high quality)
    img = cv2.resize(img, (width, height), interpolation=cv2.INTER_LANCZOS4)
    h, w = img.shape[:2]
    ...
    # Generate frames with subpixel precision
    for i in range(total_frames):
        progress = i / (total_frames - 1) if total_frames > 1 else 0
        ...
        # Transformation matrix for scaling around focus point (subpixel precision)
        M = np.array([
            [s, 0, (1 - s) * focus_x],
            [0, s, (1 - s) * focus_y]
        ], dtype=np.float32)
        
        # Apply warpAffine with Lanczos4 interpolation (high quality + subpixel)
...
def create_video_segment(
    ...
    if is_zoom:
        try:
            if CUPY_AVAILABLE:
                # CuPy GPU streaming: Generate and pipe frames directly to FFmpeg
                # Build FFmpeg command for rawvideo stdin
                cmd = [
                    FFMPEG_BIN, '-y',
                    '-f', 'rawvideo',
                    '-vcodec', 'rawvideo',
                    '-s', f'{width}x{height}',
                    '-pix_fmt', 'bgr24',  # CuPy outputs BGR
                    '-r', str(framerate),
                    '-i', '-',  # Read from stdin
                    '-c:v', ENCODER_NAME,
                ]
    ...
    # Note: For static images, we don't use -hwaccel cuda on input
    # because image decoders don't benefit from it. We use GPU filters instead.
    cmd = [
        FFMPEG_BIN, '-y',
        '-loop', '1',
        '-i', image_path,
...
def render_video_payload(payload: Dict[str, Any], progress_cb=None, step_cb=None) -> Dict[str, Any]:
    ...
    step_cb: callable(step:str) -> None for current step updates
    """
    if progress_cb is None:
        progress_cb = lambda _p: None
    if step_cb is None:
        step_cb = lambda _s: None
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Download audio
        step_cb("Téléchargement de l'audio...")
        ...
        MAX_WORKERS = 30  # Reduced to avoid I/O contention
