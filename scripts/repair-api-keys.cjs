...
async function repairApiKeys() {
  console.log('🔧 Réparation des clés API...\n');
  console.log('   Project:', PROJECT_REF);

  // Migration 1: Fix get_user_api_key and add gemini support
  const migration1 = `
-- Add gemini_api_key column
ALTER TABLE public.user_api_keys 
ADD COLUMN IF NOT EXISTS gemini_api_key TEXT;

-- Fix get_user_api_key function
...
-- Update store_user_api_key function
...
-- Update delete_user_api_key function
...
`;
  ...
  // Migration 2: Add qa_prompt column
  const migration2 = `
...
`;
  ...
  try {
    await executeSQL(migration1, 'Migration 1: Réparation get_user_api_key + support Gemini');
    await executeSQL(migration2, 'Migration 2: Ajout qa_prompt aux presets');
    
    console.log('\n🎉 Migrations appliquées avec succès !');
    console.log('\n📝 Prochaines étapes :');
    console.log('   1. Rafraîchis ta page Profil (F5)');
    console.log('   2. Tes clés API devraient réapparaître !');
    console.log('   3. Tu peux maintenant modifier les prompts QA dans les presets');
  } catch (error) {
